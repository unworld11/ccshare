...
const CACHE = path.join(require('./paths').DIR, 'update.json');
...
function runUpdate() {
  ...
  say('manycode: checking github…\n');
  const fetch = git(['fetch', 'origin']);
  if (fetch.status !== 0) {
    say('manycode: could not reach github - are you online?\n');
  ...
  if (remote && remote === oldHead) {
    ...
    say(`manycode: you're on the latest already (${oldVer}, ${oldHead.slice(0, 7)})\n`);
    process.exit(0);
  }
  ...
  if (dirty) {
    say(`manycode: you have local changes in ${ROOT} - stash or commit them, then rerun:\n`);
    for (const l of dirty.split('\n').slice(0, 8)) say('  ' + l + '\n');
    process.exit(1);
  }
  const merge = git(['merge', '--ff-only', 'origin/master']);
  if (merge.status !== 0) {
    say('manycode: your copy has diverged from github. if you have no local work worth keeping:\n');
    say(`  git -C ${ROOT} reset --hard origin/master && manycode update\n`);
    process.exit(1);
  }

  say('manycode: installing dependencies…\n');
  const install = spawnSync('npm', ['i', '--no-fund', '--no-audit'], { cwd: ROOT, encoding: 'utf8' });
  if (install.status !== 0) {
    say('manycode: npm install failed:\n');
  ...
  say('\n');
  say(newVer === oldVer
    ? `manycode: updated ${newVer} (${oldHead.slice(0, 7)} → ${newHead.slice(0, 7)})\n`
    : `manycode: updated ${oldVer} → ${newVer}\n`);
  for (const s of whatsNew(oldVer)) {
    say(`\n  new in ${s.ver}:\n`);
    for (const h of s.heads) say(`    • ${h}\n`);
  }
  say(`\n  full notes: https://github.com/unworld11/manycode/blob/master/CHANGELOG.md\n`);
  process.exit(0);
}
