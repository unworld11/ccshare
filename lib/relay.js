...
const MAX_BUFFERED = Number(process.env.MANYCODE_MAX_BUFFERED || process.env.CCSHARE_MAX_BUFFERED) || 8 * 1024 * 1024;
...
function startRelay(port) {
  ...
  const server = http.createServer((req, res) => {
    ...
    res.end(`manycode relay - ${rooms.size} active session(s)\n`);
  });
  ...
  server.listen(port, () => {
    process.stderr.write(`manycode relay listening on :${port}\n`);
  });
  return server;
}
