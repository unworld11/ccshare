...
async function host(opts) {
  ...
  const chatHistory = [];
  const chatNotify = opts.chatNotify !== false; // --no-chat-notify (GUI hosts) turns off macOS popups
  ...
  const handleChat = (fromId, fromName, text, isHost) => {
    ...
    // A GUI host (the Mac app) shows chat in its own window and is itself a
    // joiner to this session, so a macOS popup would notify it about its own
    // messages - it passes --no-chat-notify to turn these off.
    if (!isHost && chatNotify && process.platform === 'darwin') {
