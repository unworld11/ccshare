...
const BIN_DIR = path.join(require('./paths').DIR, 'bin');
const BIN = path.join(BIN_DIR, 'manycode-menubar');
...
function ensureBuilt(log) {
  ...
  try {
    ...
    if (log) log('manycode: building menu bar helper (first run, takes a few seconds)\n');
